...
%     2 is the identifier of the ending point = destination
    ...
    MazeBlind(sx,sy)=2;
    queue=[sx,sy,sqrt((sx - ex)^2 + (sy - ey)^2),0];
    exception=0;
    parent = cell(sz,sz);
    tic
    while size(queue)>0 
            ...
            for i = 1:length(directions)
             ...
             if  ~isempty(neighbors)
                for i = 1:size(neighbors)
                    ...
                    queue= [nx,ny,pathVal+1+sqrt((ex - nx)^2 + (ey - ny)^2),pathVal+1;queue];

                    % Sort the array based on the 3rd column (3rd element) in ascending order
                    queue = sortrows(queue, [3 4]);
                end
             end
             if exception ==1
                 break
...
function animation_Callback(hObject, ~, handles)
...
function turnSpeed_Callback(hObject, eventdata, handles)
% hObject    handle to turnSpeed (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of turnSpeed as text
%        str2double(get(hObject,'String')) returns contents of turnSpeed as a double
end

% --- Executes during object creation, after setting all properties.
function turnSpeed_CreateFcn(hObject, eventdata, handles)
% hObject    handle to turnSpeed (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function desacceleration_Callback(hObject, eventdata, handles)
% hObject    handle to desacceleration (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of desacceleration as text
%        str2double(get(hObject,'String')) returns contents of desacceleration as a double
end

% --- Executes during object creation, after setting all properties.
function desacceleration_CreateFcn(hObject, eventdata, handles)
% hObject    handle to desacceleration (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in diagonally.
function diagonally_Callback(hObject, eventdata, handles)
% hObject    handle to diagonally (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of diagonally
     diagonally=findobj('Tag', 'diagonally');
     global directions;
    if (diagonally.Value==1)
...
% --- Executes on button press in fastPath.
function fastPath_Callback(hObject, eventdata, handles)
% hObject    handle to fastPath (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
readymazematrix=handles.readymazematrix;
    maze=readymazematrix ;
...
%     choose ending point
    ...
    ex=sz-fix(ex)+1;
    ey=fix(ey);
%Speed at Starting point is 0 and the max Speed is 2m/s 
    

 %Speed at Starting point is 0 and the max Speed is 2m/s 
    speed=0.1*100/1.2;
    %maxSpeed=findobj('Tag', 'maxSpeed').value;
    %maxSpeed = findobj(0, 'tag', 'maxSpeed');
    maxSpeed = str2double( get(findobj(0, 'tag', 'maxSpeed'), 'string') )*100/1.2;
    virageSpeed = str2double( get(findobj(0, 'tag', 'turnSpeed'), 'string') )*100/1.2;

    
    %Acceleration is Constant 9m/s/S that 
    %micromouse will reach max Speed after 222 micro Second if it start
    %from 0, but if it was moving the less speed can be reached is 0.4m/s
    %that mean it will reach max speed after 178 micro Second
    acceleration=str2double( get(findobj(0, 'tag', 'acceleration'), 'string') )*100/1.2;
    %Desacceleration is Constant 7m/s/s
    %microMouse will decrese from 2m/s to 0.4m/s in 228 micro Second
    
    deacceleration=str2double( get(findobj(0, 'tag', 'deacceleration'), 'string') )*100/1.2;
%   2 is the identifier of the ending point = destination
    maze(ex,ey)=2;
    handles.readymazematrix=maze;
    MazeBlind(sx,sy)=2;
    queue=[sx,sy,0,speed,(sqrt(abs(sx-ex).^2+abs(ey-sy).^2)/speed),1,0];
    exception=0;
    parent = cell(sz,sz);
    tic
    while size(queue)>0 
        ...
        for k=1:size(queue,1)
            ...
            for i = 1:length(directions)
                ...
                ny = y + directions(i, 2);
                if nx > 0 && ny > 0 && nx <= sz && ny <= sz && maze(nx,ny) ~=3
                    neighbors = [neighbors; nx, ny,directions(i)];
                end
            end
             ...
             if  ~isempty(neighbors)
                for i = 1:size(neighbors)
                    ...
                    if maze(nx,ny)==2
                        destination = neighbor;
                        parent{nx, ny} = [x,y,newSpeed];
                        timePassed
                        exception =1;
                        break
                    end
                    newDirection = updateDirection([x,y], [nx,ny], [dirX,dirY]);
                    if isequal(newDirection,[dirX,dirY])
                        newSpeed=instantSpeed+acceleration*(1/instantSpeed);
                    ...
                    maze(nx,ny)=3;
                    queue=[queue;nx,ny,timePassed+(1/newSpeed),newSpeed,((abs(nx-ex)+abs(ny-ey))/maxSpeed)+timePassed+(1/instantSpeed)-(abs(sx-ex)+abs(sy-ey))/maxSpeed,newDirection(1),newDirection(2)];
                    queue = sortrows(queue, [3 5]);
